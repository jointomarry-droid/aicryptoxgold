...
function getCgCache(key) {
  const c = cgCache.get(key);
  if (c && Date.now() < c.expiry) return c.data;
  return null;
}
function setCgCache(key, data, ttl = CG_CACHE_TTL) {
  cgCache.set(key, { data, expiry: Date.now() + ttl });
}

const getMockMetalsData = () => ({
  success: true, base: "USD",
  rates: { "XAG":0.035,"XAU":0.0005,"XPD":0.001,"XPT":0.0011,"BTC":0.000015,"ETH":0.00032,"USDXAG":28.5,"USDXAU":2043.20,"USDXPD":980.50,"USDXPT":910.20,"USDBTC":64500.00,"USDETH":3200.00 }
});

// Metals endpoint - Binance + CoinGecko
app.get("/api/metals", async (_req, res) => {
  try {
    const cached = getCgCache("metals");
    ...
    try {
      ...
      rates.USDBTC = parseFloat(btcRes.data.price); rates.BTC = 1 / rates.USDBTC;
      rates.USDETH = parseFloat(ethRes.data.price); rates.ETH = 1 / rates.USDETH;
    } catch {}
    ...
    const data = { success: true, base: "USD", rates, timestamp: Date.now(), source: 'binance' };
    setCgCache("metals", data, 30000);
    res.json(data);
...
app.get("/api/coingecko/markets", async (req, res) => {
  try {
    ...
    const response = await axios.get(`${COINGECKO_BASE}/coins/markets`, {
      params: { vs_currency, order: "market_cap_desc", per_page: parseInt(per_page), page: parseInt(page), sparkline: false, price_change_percentage: "1h,24h,7d" },
      timeout: 15000,
    });
...
app.get("/api/coingecko/price", async (req, res) => {
  try {
    ...
    const cached = getCgCache(`price_${ids}`);
    if (cached) return res.json(cached);
    ...
    setCgCache(`price_${ids}`, response.data);
    res.json(response.data);
...
app.get("/api/coingecko/chart/:id", async (req, res) => {
  try {
    ...
    const response = await axios.get(`${COINGECKO_BASE}/coins/${id}/market_chart`, {
      params: { vs_currency, days: parseInt(days) },
      timeout: 15000,
    });
...
app.get("/api/binance/klines/:symbol", async (req, res) => {
  try {
    ...
    const response = await axios.get("https://api.binance.com/api/v3/klines", {
      params: { symbol: `${symbol}USDT`, interval, limit: parseInt(limit) },
      timeout: 10000,
    });
...
app.get("/api/coinbase/prices", async (_req, res) => {
  try {
    ...
    const prices = (response.data.data || []).map(p => ({ pair: p.base + "-" + p.currency, base: p.base, price: parseFloat(p.amount) }));
...
app.get("/api/cryptocompare/news", async (_req, res) => {
  try {
    ...
    const articles = (response.data.Data || []).slice(0, 15).map(a => ({
...
app.post("/api/sentiment", async (req, res) => {
  try {
    ...
    const prompt = `Analyze current market sentiment for ${asset}. Return JSON with 'sentiment' (Bullish/Bearish/Neutral), 'score' (0-100), 'summary' (2-3 sentences).`;
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: { type: Type.OBJECT, properties: { sentiment: { type: Type.STRING }, score: { type: Type.NUMBER }, summary: { type: Type.STRING } }, required: ["sentiment", "score", "summary"] }
      }
    });
    ...
    res.json({ sentiment: isCrypto ? 'Bullish' : 'Neutral', score: isCrypto ? 75 : 55, summary: `Market analysis for ${req.body.asset || 'market'}.` });
  }
});
...
app.post("/api/coingecko/recommendations", async (req, res) => {
  try {
    ...
    const [marketsRes, fearGreedRes, globalRes, binanceRes] = await Promise.allSettled([
      axios.get(`${COINGECKO_BASE}/coins/markets`, { params: { vs_currency: "usd", ids: coinIds.join(","), order: "market_cap_desc", sparkline: false, price_change_percentage: "1h,24h,7d,30d" }, timeout: 15000 }),
    ...
    const binanceMap = new Map(binanceTickers.filter(t => t.symbol.endsWith("USDT")).map(t => [t.symbol.replace("USDT", "").toLowerCase(), t]));
    const fearGreed = fearGreedData || { value: "50", value_classification: "Neutral" };
    let recommendations = coins.map(coin => {
      const bin = binanceMap.get(coin.symbol) || {};
      ...
      let action = "HOLD"; let confidence = 50;
      ...
      return {
        ...
        prices: { coingecko: coin.current_price, binance: bin.price || null, average: prices.reduce((a, b) => a + b, 0) / prices.length },
        ...
        signals: { momentum: momentumScore > 5 ? "BULLISH" : momentumScore < -5 ? "BEARISH" : "NEUTRAL", volume: volumeSurge > 0.08 ? "HIGH" : volumeSurge > 0.03 ? "NORMAL" : "LOW", volatility: Math.abs(change7d) > 15 ? "HIGH" : Math.abs(change7d) > 5 ? "MEDIUM" : "LOW", sentiment: parseInt(fearGreed.value) > 60 ? "GREEDY" : parseInt(fearGreed.value) < 40 ? "FEARFUL" : "NEUTRAL" },
      };
    });
...
app.post("/api/ai/oracle", async (req, res) => {
  try {
    ...
    const { GoogleGenAI, Type } = await import("@google/genai");
    const ai = new GoogleGenAI({ apiKey, httpOptions: { headers: { 'User-Agent': 'aistudio-build' } } });
    const prompt = `Analyze ${asset} with real-time data:
...
Fear/Greed: ${fearGreed?.value || 50}
Timeframe: ${timeframe}
Provide JSON with: price_prediction (conservative/moderate/aggressive), confidence_score, risk_assessment, key_levels (support/resistance arrays), trading_strategy, catalysts array, probability_analysis (bull/bear/neutral), news_summary`;
    const response = await ai.models.generateContent({
      ...
      config: { tools: [{ googleSearch: {} }], responseMimeType: "application/json",
        responseSchema: { type: Type.OBJECT, properties: {
          ...
          confidence_score: { type: Type.NUMBER }, risk_assessment: { type: Type.STRING },
          ...
          trading_strategy: { type: Type.STRING }, catalysts: { type: Type.ARRAY, items: { type: Type.STRING } },
  ...
  } catch { res.json({ price_prediction: { conservative: 0, moderate: 0, aggressive: 0 }, confidence_score: 50, risk_assessment: "MEDIUM", key_levels: { support: [], resistance: [] }, trading_strategy: "Analysis temporarily unavailable.", catalysts: [], probability_analysis: { bull: 33, bear: 33, neutral: 34 }, news_summary: "N/A", error: true }); }
});

// Crash Predictor
app.get("/api/ai/crash-predictor", async (_req, res) => {
  try {
    const cached = getCgCache("crash_predictor");
    if (cached) return res.json(cached);
    ...
    const prediction = riskScore > 60 ? "Significant correction likely within 48-72 hours" : riskScore > 40 ? "Elevated risk of pullback in next 1-2 weeks" : riskScore > 20 ? "Market showing some stress, monitor closely" : "Market conditions appear stable";
    const data = { riskScore, riskLevel, prediction, indicators, fearGreedIndex: parseInt(fearGreed.value), btcPrice: btc.current_price, btcChange7d: btc.price_change_percentage_7d, recommendations: riskScore > 40 ? ["Consider reducing exposure", "Set stop-losses", "Avoid leveraged positions"] : ["Market conditions favorable", "Dollar-cost averaging recommended"], timestamp: new Date().toISOString() };
    setCgCache("crash_predictor", data, 300000);
    res.json(data);
  } catch { res.status(500).json({ error: "Failed" }); }
});
...
app.get("/api/sentiment/heatmap", async (_req, res) => {
  try {
    const cached = getCgCache("sentiment_heatmap");
    if (cached) return res.json(cached);
    ...
    const heatmap = marketRes.data.map(coin => {
      ...
      return { id: coin.id, symbol: coin.symbol.toUpperCase(), name: coin.name, image: coin.image, price: coin.current_price, change1h, change24h, change7d, sentiment, intensity: Math.round(intensity), volumeRatio: (volumeRatio * 100).toFixed(1), marketCap: coin.market_cap };
    });
    const data = { assets: heatmap, timestamp: new Date().toISOString() };
    setCgCache("sentiment_heatmap", data, 60000);
    res.json(data);
  } catch { res.status(500).json({ error: "Failed" }); }
});
...
app.get("/api/correlation/matrix", async (_req, res) => {
  try {
    const cached = getCgCache("correlation");
    if (cached) return res.json(cached);
    const assets = ['bitcoin', 'ethereum', 'solana', 'binancecoin', 'ripple'];
    const charts = await Promise.allSettled(assets.map(id => axios.get(`${COINGECKO_BASE}/coins/${id}/market_chart?vs_currency=usd&days=30`, { timeout: 15000 })));
    const priceSeries = {};
    charts.forEach((r, i) => { if (r.status === 'fulfilled') priceSeries[assets[i]] = r.value.data.prices.map(p => p[1]); });
    const calcCorr = (a, b) => {
    ...
    const matrix = {};
    for (const a of assets) { matrix[a] = {}; for (const b of assets) matrix[a][b] = a === b ? 1 : calcCorr(priceSeries[a] || [], priceSeries[b] || []); }
    const data = { matrix, assets, timestamp: new Date().toISOString() };
    setCgCache("correlation", data, 600000);
    res.json(data);
  } catch { res.status(500).json({ error: "Failed" }); }
});
...
app.get("/api/arbitrage/scanner", async (_req, res) => {
  try {
    const cached = getCgCache("arbitrage");
    if (cached) return res.json(cached);
    const symbols = ['BTCUSDT', 'ETHUSDT', 'SOLUSDT', 'BNBUSDT', 'XRPUSDT'];
    const opportunities = [];
    for (const symbol of symbols) {
      const [binRes, cbRes] = await Promise.allSettled([
        axios.get(`https://api.binance.com/api/v3/ticker/price?symbol=${symbol}`, { timeout: 5000 }),
        axios.get(`https://api.coinbase.com/v2/prices/${symbol.replace('USDT', '-USD')}/spot`, { timeout: 5000 })
      ]);
      const prices = {};
      if (binRes.status === 'fulfilled') prices.Binance = parseFloat(binRes.value.data.price);
      if (cbRes.status === 'fulfilled') prices.Coinbase = parseFloat(cbRes.value.data.amount);
      const vals = Object.values(prices);
      if (vals.length >= 2) {
        const low = Math.min(...vals), high = Math.max(...vals);
        const spread = ((high - low) / low) * 100;
        const buyEx = Object.entries(prices).find(([,v]) => v === low)?.[0] || '';
        const sellEx = Object.entries(prices).find(([,v]) => v === high)?.[0] || '';
        opportunities.push({ symbol: symbol.replace('USDT', ''), prices, spread: spread.toFixed(3), profitPer1000: ((1000 / low) * high - 1000).toFixed(2), buyOn: buyEx, sellOn: sellEx, action: spread > 0.1 ? 'OPPORTUNITY' : 'NO_ARB' });
      }
    }
    opportunities.sort((a, b) => parseFloat(b.spread) - parseFloat(a.spread));
    const data = { opportunities, timestamp: new Date().toISOString() };
    setCgCache("arbitrage", data, 15000);
    res.json(data);
  } catch { res.status(500).json({ error: "Failed" }); }
});
...
app.post("/api/ai/trading-signals", async (req, res) => {
  try {
    ...
    const prices = chartRes.status === "fulfilled" ? chartRes.value.data.prices.map(p => p[1]) : [];
    ...
    if (prices.length > 14) {
      ...
      for (let i = prices.length - 14; i < prices.length; i++) { const d = prices[i] - prices[i-1]; if (d > 0) gains += d; else losses -= d; }
      const rs = losses === 0 ? 100 : gains / losses;
      rsi = 100 - (100 / (1 + rs));
    }
    const { GoogleGenAI, Type } = await import("@google/genai");
    const ai = new GoogleGenAI({ apiKey, httpOptions: { headers: { 'User-Agent': 'aistudio-build' } } });
    const prompt = `Generate trading signals for ${asset}: Price $${currentPrice}, 24h ${ticker.priceChangePercent || 'N/A'}%, SMA20 $${sma20.toFixed(2)}, SMA50 $${sma50.toFixed(2)}, RSI ${rsi.toFixed(2)}, Volume ${ticker.quoteVolume || 'N/A'}. Return JSON with: signal, entry_price, stop_loss, take_profit_1, take_profit_2, risk_reward_ratio, timeframe, indicators (rsi_signal, macd_signal, trend), confidence, reasoning`;
    const response = await ai.models.generateContent({
      model: "gemini-2.0-flash", contents: prompt,
      config: { responseMimeType: "application/json",
        responseSchema: { type: Type.OBJECT, properties: {
          signal: { type: Type.STRING }, entry_price: { type: Type.NUMBER }, stop_loss: { type: Type.NUMBER },
          take_profit_1: { type: Type.NUMBER }, take_profit_2: { type: Type.NUMBER }, risk_reward_ratio: { type: Type.NUMBER },
          timeframe: { type: Type.STRING }, indicators: { type: Type.OBJECT }, confidence: { type: Type.NUMBER }, reasoning: { type: Type.STRING }
...
app.post("/api/ai/portfolio/optimize", async (req, res) => {
  try {
    ...
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) return res.status(500).json({ error: "GEMINI_API_KEY not configured" });
    const ids = portfolio?.map(p => p.coinId).join(',') || 'bitcoin,ethereum,solana';
    ...
    const { GoogleGenAI, Type } = await import("@google/genai");
    const ai = new GoogleGenAI({ apiKey, httpOptions: { headers: { 'User-Agent': 'aistudio-build' } } });
    const prompt = `Optimize crypto portfolio with risk tolerance: ${riskTolerance}. Holdings: ${JSON.stringify(portfolio)}. Prices: ${JSON.stringify(coins.map(c => ({ id: c.id, price: c.current_price, change24h: c.price_change_percentage_24h })))}. Return JSON with: optimized_allocation, expected_return, risk_score, rebalance_actions, diversification_score, sharpe_ratio_estimate, max_drawdown_estimate, reasoning`;
    const response = await ai.models.generateContent({
      model: "gemini-2.0-flash", contents: prompt,
      config: { responseMimeType: "application/json",
        responseSchema: { type: Type.OBJECT, properties: {
          optimized_allocation: { type: Type.OBJECT }, expected_return: { type: Type.STRING }, risk_score: { type: Type.NUMBER },
          rebalance_actions: { type: Type.ARRAY, items: { type: Type.OBJECT } }, diversification_score: { type: Type.NUMBER },
          sharpe_ratio_estimate: { type: Type.NUMBER }, max_drawdown_estimate: { type: Type.STRING }, reasoning: { type: Type.STRING }
